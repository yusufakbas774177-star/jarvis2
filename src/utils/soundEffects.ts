// Web Audio API Synthesizer for Stark / J.A.R.V.I.S. Audio Feedback
...
// Gentle UI Click / Chirp
export function playChirp(pitch = 1200) {
...
// System Command Confirmation Chime
export function playCommandSuccess() {
  if (!soundEnabled) return;
  try {
    const ctx = getAudioContext();
    if (!ctx) return;
